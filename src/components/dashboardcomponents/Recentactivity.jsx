...
const fetchUserrecentdata = async (username) => {
    try {
        ...
        messages: messages[index]?.map((commit) => commit.message) || "no-commit-message",
    }));
    
    setCommitmessage(messages);
        ...
        {recentdata.map((ele, idx) => {
          return (
              <div
                ...
                key={idx}
                >
                <img title={ele.type} className="h-5 hidden md:inline w-5 ms-25" src={eventIcons[ele.type]||CreateEvent} alt={ele.type} />
